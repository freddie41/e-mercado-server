...
//Variable para guardar todos los datos de la orden.
var newOrder;
...
//Función para guardar usuario que genera nueva orden.
...
.on("click", "#submitCart", function (event) {
  ...
  if (!validateForms()) {
    ...
    //Guarda los datos para enviar al servidor.
    newOrder = {
      "user": showUserLogged(),
      "dateTime": new Date().toLocaleString(),
      "products": JSON.stringify(cartProducts),
      "shippingType": $('input[name="shippingType"]:checked').next().html(),
      "shippingCost": "USD " + $('#shippingCost').html(),
      "subtotal": "USD " + $('#subtotalCarrito').html(),
      "total": "USD " + $('#totalCarrito').html(),
      "name": $('#firstName').val(),
      ...
      "state": $('#state').val(),
      "zip": $('#zip').val(),
      "paymentType": $('input[name="paymentType"]:checked').next().html()
    }

    //Envia los datos al servidor para guardar en archivo txt local.
    postJSONData(POST_NEW_ORDER, newOrder).then(function (resultObj) {
      if (resultObj.status === "ok") {

        //Alerta de confirmación de tipo modal al crear la orden con exito.
        swalBSStandardBtn.fire({
          title: '¡Éxito!',
          html: resultObj.data.msj,
          icon: 'success',
          confirmButtonText: 'Aceptar',
        });
        ...
        //Alerta de confirmación de tipo modal si el servidor arroja msj de error.
        swalBSStandardBtn.fire({
          title: '¡Ups!',
          html: resultObj.data.msj,
          icon: 'error',
